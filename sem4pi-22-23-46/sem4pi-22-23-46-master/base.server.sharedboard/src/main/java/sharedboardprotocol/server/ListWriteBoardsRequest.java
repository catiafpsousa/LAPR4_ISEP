...
public class ListWriteBoardsRequest extends Thread {
    private SBPMessage result;
    private ServerCreatePostItController controller;
    private SystemUser user;

    public ListWriteBoardsRequest(ServerCreatePostItController controller, SystemUser user) {
    ...
    public void run() {
        try {
            List<SharedBoard> sharedBoardList = (List) controller.sharedBoardsWithWritePermission(user);
